...
const char *log_FileName = "ETS2Data.txt";
...
void cleanLog()
{
	...
	fileLength = ftell(log_file);

	if (fileLength >= 1000)
	{
		freopen(log_FileName, "w", log_file);
		fprintf(log_file, "");
		freopen(log_FileName, "w+", log_file);
	}
}
...
SCSAPI_VOID telemetry_frame_start(const scs_event_t UNUSED(event), const void *const event_info, const scs_context_t UNUSED(context))
{
	...
	data.timestamp += (info->paused_simulation_time - last_timestamp);
	last_timestamp = info->paused_simulation_time;

	// The raw values.

	data.raw_rendering_timestamp = info->render_time;
	data.raw_simulation_timestamp = info->simulation_time;
	data.raw_paused_simulation_timestamp = info->paused_simulation_time;*/
}
...
SCSAPI_VOID telemetry_frame_end(const scs_event_t UNUSED(event), const void *const UNUSED(event_info), const scs_context_t UNUSED(context))
{
	...
	//log_print("%" SCS_PF_U64 ";%" SCS_PF_U64 ";%" SCS_PF_U64 ";%" SCS_PF_U64, data.timestamp, data.raw_rendering_timestamp, data.raw_simulation_timestamp, data.raw_paused_simulation_timestamp);

	if (data.orientation_available) {
		log_print("%f;%f;%f", data.heading, data.pitch, data.roll);
	}

	log_line( // Last thing to print, so add newline(\n).
		";%f;%f;%d",
		data.speed,
		data.rpm,
		data.gear
...
SCSAPI_RESULT scs_telemetry_init(const scs_u32_t version, const scs_telemetry_init_params_t *const params)
{
	...
	if (! events_registered) {
		...
		version_params->common.log(SCS_LOG_TYPE_error, "WARNING: Unable to register event callbacks");
		return SCS_RESULT_generic_error;
	}
	...
	version_params->register_for_channel(SCS_TELEMETRY_TRUCK_CHANNEL_world_placement, SCS_U32_NIL, SCS_VALUE_TYPE_euler, SCS_TELEMETRY_CHANNEL_FLAG_no_value, telemetry_store_orientation, &data);
	version_params->register_for_channel(SCS_TELEMETRY_TRUCK_CHANNEL_speed, SCS_U32_NIL, SCS_VALUE_TYPE_float, SCS_TELEMETRY_CHANNEL_FLAG_none, telemetry_store_float, &data.speed);
	version_params->register_for_channel(SCS_TELEMETRY_TRUCK_CHANNEL_engine_rpm, SCS_U32_NIL, SCS_VALUE_TYPE_float, SCS_TELEMETRY_CHANNEL_FLAG_none, telemetry_store_float, &data.rpm);
	version_params->register_for_channel(SCS_TELEMETRY_TRUCK_CHANNEL_engine_gear, SCS_U32_NIL, SCS_VALUE_TYPE_s32, SCS_TELEMETRY_CHANNEL_FLAG_none, telemetry_store_s32, &data.gear);

	// Remember the function we will use for logging.

	game_log = version_params->common.log;
	game_log(SCS_LOG_TYPE_message, "Initializing ETS2Viewer");
